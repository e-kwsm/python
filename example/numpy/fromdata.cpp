...
/**
 *  @brief An example to show how to access data using raw pointers.  This shows
 *         that you can use and manipulate data in either Python or C++ and have
 *         the changes reflected in both.
 */
...
int main(int argc, char **argv) {
  ...
  std::cout << std::endl
            << "Python ndarray :" << p::extract<char const *>(p::str(py_array))
            << std::endl;
  // Change an element in the python ndarray
  py_array[1] = 5;
  // And see if the C++ container is changed or not
  std::cout << "Is the change reflected in the C++ array used to create the "
               "ndarray ? "
            << std::endl;
  for (int j = 0; j < 4; j++) {
    std::cout << arr[j] << ' ';
