...
// This is roughly the most efficient way to write a C/C++ function that
// operates on a 2-d NumPy array - operate directly on the array by incrementing
// a pointer with the strides.
...
void wrap_fill1(np::ndarray const &array) {
  ...
  fill1(reinterpret_cast<double *>(array.get_data()), array.shape(0),
        array.shape(1), array.strides(0) / sizeof(double),
