...
public:
  explicit pymutex_guard(pymutex &mutex) : m_mutex(mutex) { m_mutex.lock(); }

  ~pymutex_guard() { m_mutex.unlock(); }
...
#define BOOST_PYTHON_LOCK_STATE()                                              \
  ::boost::python::detail::pymutex_guard lock(                                 \
      ::boost::python::detail::get_global_mutex())
