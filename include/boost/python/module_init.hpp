...
#define _BOOST_PYTHON_MODULE_INIT(name, ...)                                   \
  PyObject *BOOST_PP_CAT(PyInit_, name)() {                                    \
    ...
    static struct PyModuleDef moduledef = {                                    \
        initial_m_base,                                                        \
        BOOST_PP_STRINGIZE(name), 0,           /* m_doc */                     \
                           -1,                 /* m_size */                    \
                           initial_methods, 0, /* m_reload */                  \
                           0,                  /* m_traverse */                \
                           0,                  /* m_clear */                   \
                           0,                  /* m_free */                    \
        };                                                                     \
...
#define _BOOST_PYTHON_MODULE_MULTI_PHASE_INIT(name, ...)                       \
  ...
  extern "C" BOOST_SYMBOL_EXPORT PyObject *BOOST_PP_CAT(PyInit_, name)() {     \
    ...
    static PyModuleDef_Slot slots[] = {                                        \
        {Py_mod_exec, reinterpret_cast<void *>(reinterpret_cast<uintptr_t>(    \
                          BOOST_PP_CAT(exec_module_, name)))},                 \
        {Py_mod_gil, boost::python::detail::gil_not_used_option(__VA_ARGS__)   \
                         ? Py_MOD_GIL_NOT_USED                                 \
                         : Py_MOD_GIL_USED},                                   \
        {0, NULL}};                                                            \
                                                                               \
...
#define _BOOST_PYTHON_MODULE_WITH_STATE_INIT(name, StateType, ...)             \
  int BOOST_PP_CAT(exec_module_, name)(PyObject * mod) {                       \
    return boost::python::detail::exec_module_with_state(                      \
        mod,                                                                   \
        reinterpret_cast<void (*)(void *)>(BOOST_PP_CAT(init_module_, name))); \
  }                                                                            \
  extern "C" BOOST_SYMBOL_EXPORT PyObject *BOOST_PP_CAT(PyInit_, name)() {     \
...
#define BOOST_PYTHON_MODULE_MULTI_PHASE_INIT(name, ...)                        \
  void BOOST_PP_CAT(init_module_, name)();                                     \
  extern "C" BOOST_SYMBOL_EXPORT _BOOST_PYTHON_MODULE_MULTI_PHASE_INIT(        \
      name, __VA_ARGS__)
#define BOOST_PYTHON_MODULE_WITH_STATE_INIT(name, StateType, ...)              \
  void BOOST_PP_CAT(init_module_, name)(StateType * state);                    \
  extern "C" BOOST_SYMBOL_EXPORT _BOOST_PYTHON_MODULE_WITH_STATE_INIT(         \
      name, StateType, __VA_ARGS__)
#else
#define BOOST_PYTHON_MODULE_MULTI_PHASE_INIT(name)                             \
  void BOOST_PP_CAT(init_module_, name)();                                     \
