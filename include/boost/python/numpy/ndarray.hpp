 ...
 *        boost::python::numeric::array).  Right now all that exists is what was
 *        needed to move raw data between C++ and Python.
 */
...
public:
   ...
   *        to not make these simple integer values for overloading purposes,
   *        but the need to define every possible combination and custom bitwise
   *        operators is ugly.
   */
  enum bitflag {
    NONE = 0x0,
  ...
  /// @brief Return the stride of the array as an array of integers
  ///        (length == get_nd()).
 ...
 *                     dtype::get_builtin().
 *  @param[in] shape   Shape of the array as STL container of integers;
 *                     must have begin() and end().
 *  @param[in] strides Shape of the array as STL container of integers;
 *                     must have begin() and end().
 *  @param[in] owner   An arbitray Python object that owns that data pointer.
 *                     The array object will keep a reference to the object, and
 *                     decrement it's reference count when the array goes out of
 *                     scope.  Pass None at your own peril.
 *
