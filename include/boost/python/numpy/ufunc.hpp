 ...
 *        should use that name. It's more dangerous than most object managers,
 *        however - maybe it actually belongs in a detail namespace?
 */
 ...
 *    typedef double result_type;
 *    double operator()(double input1, double input2) const {
 *      return std::cos(input1 + input2);
 *    }
 *  };
 *
 *  BOOST_PYTHON_MODULE(example)
