...
private:
  static size_t base_count(Container &container, object v) {
  ...
  static object base_iadd(Container &container, object v) {
    base_extend(container, v);
    return object(container);
  }

  static void base_remove(Container &container, object v) {
    extract<data_type &> key(v);
    if (key.check()) {
      auto i = std::find(container.begin(), container.end(), key());
      if (i == container.end()) {
        PyErr_SetString(PyExc_ValueError,
                        "remove(x): x not in vector_indexing_suite");
        throw_error_already_set();
      }
      container.erase(i);
    } else {
  ...
  static void base_reverse(Container &container) {
    using std::swap;
