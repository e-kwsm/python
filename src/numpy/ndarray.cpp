...
{
  if (get_struct()->base == BOOST_NULLPTR) return object();
...
{
  return ndarray(python::detail::new_reference
    (PyArray_Transpose(reinterpret_cast<PyArrayObject*>(this->ptr()), BOOST_NULLPTR)));
}
...
{
  return ndarray(python::detail::new_reference
    (PyArray_FromAny(obj.ptr(), BOOST_NULLPTR, 0, 0, NPY_ARRAY_ENSUREARRAY, BOOST_NULLPTR)));
}

ndarray array(python::object const & obj, dtype const & dt) 
{
  return ndarray(python::detail::new_reference
    (PyArray_FromAny(obj.ptr(), detail::incref_dtype(dt), 0, 0, NPY_ARRAY_ENSUREARRAY, BOOST_NULLPTR)));
}
