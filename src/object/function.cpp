...
object function::signature(bool show_return_type) const {
  ...
  if (show_return_type)
    return "%s(%s) -> %s" % make_tuple(m_name, str(", ").join(formal_params),
...
void function::add_to_namespace(object const &name_space, char const *name_,
  ...
  if (attribute.ptr()->ob_type == &function_type) {
    ...
    assert(!PyErr_Occurred());
    handle<> name_space_name(allow_null(
        ::PyObject_GetAttrString(name_space.ptr(), const_cast<char *>(
...
BOOST_PYTHON_DECL object const &add_doc(object const &attribute,
                                        char const *doc) {
#if PY_VERSION_HEX >= 0x03000000
