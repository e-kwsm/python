...
static str get_qualname(const PyTypeObject *py_type) {
  ...
  if (py_type->tp_flags & Py_TPFLAGS_HEAPTYPE)
    return str(
        handle<>(borrowed(((PyHeapTypeObject *)(py_type))->ht_qualname)));
#endif
  return str(py_type->tp_name);
}

str function_doc_signature_generator::py_type_str(
    const python::detail::signature_element &s,
    const object &current_module_name) {
  ...
  if (py_type) {
    ...
    if (py_type->tp_flags & Py_TPFLAGS_HEAPTYPE) {
      ...
      if (PyDict_GetItemStringRef(py_type->tp_dict, "__module__",
                                  &type_module_name) < 0) {
...
#endif
      if (type_module_name &&
          PyObject_RichCompareBool(type_module_name, current_module_name.ptr(),
                                   Py_NE) != 0) {
        str result =
            str("%s.%s" % make_tuple(handle<>(type_module_name), name));
        return result;
      }
...
str function_doc_signature_generator::parameter_string(
    py_function const &f, size_t n, object arg_names,
    const object &current_module_name, bool cpp_types) {
  str param;
  ...
  if (cpp_types) {
           ...
           // for it
    {
      ...
      if (arg_names && (kv = arg_names[n - 1]))
        param = str(" (%s)%s" %
                    make_tuple(py_type_str(s[n], current_module_name), kv[0]));
      else
        param =
            str(" (%s)%s%d" %
                make_tuple(py_type_str(s[n], current_module_name), "arg", n));
      ...
      if ((*fi)->doc()) {
        ...
        bool show_cpp_signature =
            ...
            str(detail::cpp_signature_tag) ==
                func_doc.slice(
                    1 - int(sizeof(detail::cpp_signature_tag) / sizeof(char)),
                    _);
