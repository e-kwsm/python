...
template <class Tuple> struct select1st {
...
struct q_elt {
  q_elt(std::size_t distance, void *src_address, vertex_t target,
        cast_function cast)
      : distance(distance), src_address(src_address), target(target),
        cast(cast) {}
...
inline void *convert_type(void *const p, class_id src_t, class_id dst_t,
  ...
  void *result =
      search(g, p, tuples::get<kvertex>(*src_p), tuples::get<kvertex>(*dst_p));
