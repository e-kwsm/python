...
BOOST_PYTHON_DECL object getattr(object const& target, object const& key, object const& default_)
{
    ...
    if (result == BOOST_NULLPTR && PyErr_ExceptionMatches(PyExc_AttributeError))
    {
        PyErr_Clear();
        return default_;
    }
...
#endif
      {
          PyObject *slice = PySlice_New(v, w, BOOST_NULLPTR);
          if (slice != BOOST_NULLPTR) {
...
{
    if (assign_slice(
            target.ptr(), begin.get(), end.get(), BOOST_NULLPTR) == -1
        )
    {
        throw_error_already_set();
    }
