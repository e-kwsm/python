...
bool operator==(const A &v1, const A &v2) { return (v1.value == v2.value); }
...
struct AFromPython {
  ...
  construct(PyObject *obj_ptr,
    ...
    void *storage =
        ((boost::python::converter::rvalue_from_python_storage<A> *)data)
            ->storage.bytes;
...
void a_map_indexing_suite() {
  ...
  class_<std::map<int, A>>("AMap").def(
      map_indexing_suite<std::map<int, A>, true>());

  class_<B>("B").add_property(
      "a", make_getter(&B::a, return_value_policy<return_by_value>()),
