...
int apply_int_int(PyObject *f, int x) { return call<int>(f, x); }

void apply_void_int(PyObject *f, int x) { call<void>(f, x); }

struct X {
  explicit X(int x) : x(x), magic(7654321) { ++counter; }
  X(X const &rhs) : x(rhs.x), magic(7654321) { ++counter; }
  ~X() {
    BOOST_ASSERT(magic == 7654321);
    magic = 6666666;
    x = 9999;
    --counter;
  }
...
X apply_X_X(PyObject *f, X x) { return call<X>(f, x); }

void apply_void_X_ref(PyObject *f, X &x) { call<void>(f, boost::ref(x)); }

X &apply_X_ref_handle(PyObject *f, handle<> obj) { return call<X &>(f, obj); }
...
void apply_void_X_ptr(PyObject *f, X *x) { call<void>(f, ptr(x)); }

void apply_void_X_deep_ptr(PyObject *f, X *x) { call<void>(f, x); }
...
char apply_char_char(PyObject *f, char c) { return call<char>(f, c); }
...
object apply_object_object(PyObject *f, object x) { return call<object>(f, x); }
