...
//
///////////////////////////////////////////////////////////////////////////////
object bar(int a, char b, std::string c, double d) {
  return format % bpl::make_tuple(a, b, c, d);
}

object bar(int a, char b, std::string c) {
  return format % bpl::make_tuple(a, b, c, 0.0);
}

object bar(int a, char b) {
  return format % bpl::make_tuple(a, b, "default", 0.0);
}

object bar(int a) { return format % bpl::make_tuple(a, 'D', "default", 0.0); }
...
//
///////////////////////////////////////////////////////////////////////////////
object foo(int a, char b = 'D', std::string c = "default", double d = 0.0) {
  return format % bpl::make_tuple(a, b, c, d);
}
...
struct Y {
  ...
  object get_state() const { return format % bpl::make_tuple(a, b, c, d); }
...
struct X {
  ...
  X(std::string s, bool b)
      : state("Got exactly two arguments from constructor: string(%s); "
              "bool(%s); " %
              bpl::make_tuple(s, b * 1)) {}
  ...
  Y const &bar2(int a = 0, char b = 'D', std::string c = "default",
                double d = 0.0) {
  ...
  object get_state() const { return state; }
...
BOOST_PYTHON_MODULE(defaults_ext) {
  ...
  class_<X>("X", no_init)
      ...
      .def(init<optional<int, char, std::string, double>>(
          "doc of init", args("self", "a", "b", "c", "d")))
      .def(init<std::string, bool>(
          args("self", "s",
               "b"))[default_call_policies()]) // what's a good policy here?
      .def("get_state", &X::get_state)
      .def("bar", &X::bar, X_bar_stubs())
      .def("bar2", &X::bar2,
           X_bar_stubs2("doc of X::bar2")[return_internal_reference<>()])
