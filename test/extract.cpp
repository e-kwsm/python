...
#pragma warning(push)
#pragma warning(                                                               \
    disable : 4172) // msvc lies about returning a reference to temporary
...
#if defined(BOOST_MSVC) && BOOST_MSVC <= 1300 ||                               \
    defined(_MSC_VER) && defined(__ICL) && __ICL <= 800
#pragma warning(pop)
#endif
}

X extract_X(object x) { return extract<X>(x); }
...
bool check_string_cref(object x) {
  return extract<std::string const &>(x).check();
}
...
BOOST_PYTHON_MODULE(extract_ext) {
  ...
  def("extract_string_cref", extract_string_cref,
      return_value_policy<reference_existing_object>());
  def("extract_X", extract_X);
  def("extract_X_ptr", extract_X_ptr,
      return_value_policy<reference_existing_object>());
  def("extract_X_ref", extract_X_ref,
      return_value_policy<reference_existing_object>());
  ...
  object x_class(class_<X>("X", init<int>()).def("__repr__", x_rep));
