...
void push_list_back(list_list &x, list_int const &y) { x.push_back(y); }
...
struct two_lists {
  two_lists() {
    ...
    std::copy(primes, primes + sizeof(primes) / sizeof(*primes),
              std::back_inserter(one));
    int evens[] = {2, 4, 6, 8, 10, 12};
    std::copy(evens, evens + sizeof(evens) / sizeof(*evens),
...
BOOST_PYTHON_MODULE(iterator_ext) {
  ...
  class_<list_range>("list_range")
      ...
      .def("__iter__", range(&list_range::first, &list_range::second));
  ...
  class_<two_lists>("two_lists")
      ...
      .add_property("primes", range(&two_lists::one_begin, &two_lists::one_end))
