...
struct Foo {
  Foo(int a = 0, double b = 0, const std::string &n = std::string())
      : a_(a), b_(b), n_(n) {}
...
struct Bar {
  Bar(int a = 0, double b = 0, const std::string &n = std::string())
      : a_(a), b_(b), n_(n) {}
...
BOOST_PYTHON_MODULE(keywords) {
  ...
  class_<Foo>("Foo",
              init<int, double, const std::string &>(
                  (arg("a") = 0, arg("b") = 0.0, arg("n") = std::string())))
  ...
  class_<Bar>("Bar", init<optional<int, double, const std::string &>>())
