...
extern "C" void dealloc(PyObject *self) { PyObject_Del(self); }
...
PyTypeObject NoddyType = {
    PyVarObject_HEAD_INIT(NULL, 0) const_cast<char *>("Noddy"),
    sizeof(NoddyObject),
    0,
    dealloc, /* tp_dealloc */
    0,       /* tp_print */
...
PyTypeObject SimpleType = {
    PyVarObject_HEAD_INIT(NULL, 0) const_cast<char *>("Simple"),
    sizeof(SimpleObject),
    0,
    dealloc, /* tp_dealloc */
    0,       /* tp_print */
...
struct simple_to_python : to_python_converter<simple, simple_to_python, true>
...
struct int_from_noddy {
  static int &execute(NoddyObject &p) { return p.x; }
};
...
int f(simple const &s) { return strlen(s.s); }

int f_mutable_ref(simple &s) { return strlen(s.s); }

int f_mutable_ptr(simple *s) { return strlen(s->s); }

int f_const_ptr(simple const *s) { return strlen(s->s); }

int f2(SimpleObject const &s) { return strlen(s.x.s); }

// A trivial passthru function for simple objects
simple const &g(simple const &x) { return x; }
...
BOOST_PYTHON_MODULE(m1) {
  ...
  lvalue_from_pytype<
    ...
    BOOST_MSVC > 1300 // doesn't support non-type member pointer parameters
  ...
  def("make_unregistered2", make_unregistered2,
  ...
  class_<A, shared_ptr<A>>("A").def("name", &A::name);

  // sequence points don't ensure that "A" is constructed before "B"
  // or "C" below if we make them part of the same chain
  class_<B, bases<A>>("B").def("name", &B::name);

  class_<C, bases<A>>("C").def("name", &C::name);

  class_<D, bases<B, C>>("D").def("name", &D::name);

  class_<complicated>("complicated", init<simple const &, int>())
