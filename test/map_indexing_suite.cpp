...
std::string x_value(X const &x) { return "gotya " + x.s; }
...
BOOST_PYTHON_MODULE(map_indexing_suite_ext) {
  ...
  class_<std::map<std::string, X>>("XMap").def(
      map_indexing_suite<std::map<std::string, X>>());
  ...
  // Some more..
  class_<std::map<std::string, boost::shared_ptr<X>>>("TestMap").def(
      map_indexing_suite<std::map<std::string, boost::shared_ptr<X>>, true>());
