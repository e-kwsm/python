...
struct world_pickle_suite : boost::python::pickle_suite {
  ...
  static boost::python::tuple getstate(boost::python::object w_obj) {
    ...
    return boost::python::make_tuple(w_obj.attr("__dict__"),
                                     w.get_secret_number());
  }

  static void setstate(boost::python::object w_obj,
    ...
    if (len(state) != 2) {
      PyErr_SetObject(
          PyExc_ValueError,
          ("expected 2-item tuple in call to __setstate__; got %s" % state)
              .ptr());
