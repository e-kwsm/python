...
struct PCallback : P, Callback {
  ...
  std::string f() { return call_method<std::string>(mSelf, "f"); }
};
...
struct ACallback : A, Callback {
  ...
  std::string default_f() { return A::f(); }
};
...
BOOST_PYTHON_MODULE_INIT(polymorphism_ext) {
  class_<A, boost::noncopyable, ACallback>("A").def("f", &A::f,
                                                    &ACallback::default_f);

  def("getBCppObj", getBCppObj,
      return_value_policy<reference_existing_object>());

  class_<C, bases<A>, boost::noncopyable>("C").def("f", &C::f);
  ...
  class_<P, boost::noncopyable, PCallback>("P").def("f", pure_virtual(&P::f));

  class_<Q, bases<P>>("Q").def(
      "g", &P::g) // make sure virtual inheritance doesn't interfere
      ;
}
