...
#endif
{
  class_<ACallback HELD_PTR(A), boost::noncopyable>("A").def(
      "f", &A::f, &ACallback::default_f);

  def("getBCppObj", getBCppObj,
      return_value_policy<reference_existing_object>());

  class_<C HELD_PTR(C), bases<A>, boost::noncopyable>("C").def("f", &C::f);
  ...
  class_<PCallback, boost::noncopyable>("P").def("f", pure_virtual(&P::f));

  class_<Q HELD_PTR(Q), bases<P>>("Q").def(
