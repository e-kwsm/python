...
struct X {
  X(int value) : m_value(value) { ++s_count; }

  X(const X &other) : m_value(other.m_value) { ++s_count; }

  ~X() { --s_count; }

  int get_value() const { return m_value; }

  void set_value(int new_value) { m_value = new_value; }

  static int get_instance_count() { return s_count; }
...
int get_X_instance_count() { return X::get_instance_count(); }
...
BOOST_PYTHON_MODULE(properties_ext) {
  ...
  class_<X>("X", init<int>())
      ...
      // defining read \ write property for static member using make_getter and
      // make_setter
      .add_static_property(
          "instance_count_direct",
  ...
  crash_me_wrapper.def("get_i", &test::crash_me::get_i,
