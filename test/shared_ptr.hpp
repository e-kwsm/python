...
template <class T> struct functions {
  ...
  static void store(shared_ptr<T> x) { storage = x; }

  static void release_store() { store(shared_ptr<T>()); }

  static void modify(shared_ptr<T> &x) { x.reset(); }

  static shared_ptr<T> get() { return storage; }
  static shared_ptr<T> &get1() { return storage; }

  static int look_store() { return look(get()); }

  template <class C> static void expose(C const &c) {
    def("look", &look);
    def("store", &store);
    def("modify", &modify);
...
struct ZWrap : Z {
  ZWrap(PyObject *self, int x) : Z(x), m_self(self) {}
...
shared_ptr<A> New(bool make) { return shared_ptr<A>(make ? new B() : 0); }
...
BOOST_PYTHON_MODULE(MODULE) {
  ...
  objects::class_value_wrapper<
      shared_ptr<A>, objects::make_ptr_instance<
                         A, objects::pointer_holder<shared_ptr<A>, A>>>();
  ...
  functions<X>::expose(
      class_<X, boost::noncopyable>("X", init<int>()).def("value", &X::value));

  functions<Y>::expose(
      class_<Y, shared_ptr<Y>>("Y", init<int>()).def("value", &Y::value));

  class_<YY, bases<Y>, boost::noncopyable>("YY", init<int>());

  class_<YYY, shared_ptr<YYY>, bases<Y>>("YYY", init<int>());

  functions<Z>::expose(class_<Z, ZWrap>("Z", init<int>())
                           .def("value", &Z::value)
                           .def("v", &Z::v, &ZWrap::default_v));

  // from Neal Becker
  class_<Test>("Test").def_readonly("x", &Test::x, "x");
  // ------
}
