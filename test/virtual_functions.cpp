...
struct abstract_callback : abstract {
  abstract_callback(PyObject *p, int x) : abstract(x), self(p) {}

  int f(Y const &y) { return call_method<int>(self, "f", boost::ref(y)); }
...
struct concrete_callback : concrete {
  concrete_callback(PyObject *p, int x) : concrete(x), self(p) {}

  concrete_callback(PyObject *p, concrete const &x) : concrete(x), self(p) {}

  int f(Y const &y) { return call_method<int>(self, "f", boost::ref(y)); }

  int f_impl(Y const &y) { return this->concrete::f(y); }
...
BOOST_PYTHON_MODULE(virtual_functions_ext) {
  ...
  class_<abstract, boost::noncopyable, abstract_callback>("abstract",
                                                          init<int>())
  ...
  class_<Y>("Y", init<int>()).def("value", &Y::value).def("set", &Y::set);
}
